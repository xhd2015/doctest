---
label: heavy
explanation: nested doctest test thrice; third run uses --no-leaf-cache
---
...
- Run2 warm: **Cached > 0**.
- Run3 with `--no-leaf-cache`: **0 Cached**.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if !stdoutCachedPositive(resp.Stdout2) {
		t.Fatalf("run2 warm must Cached > 0 before testing --no-leaf-cache; got %d\nstdout2:\n%s", cachedCount(resp.Stdout2), resp.Stdout2)
	}
	if !stdoutCachedZero(resp.Stdout3) {
		t.Fatalf("--no-leaf-cache must yield 0 Cached; got %d\nstdout3:\n%s", cachedCount(resp.Stdout3), resp.Stdout3)
	}
}
```
