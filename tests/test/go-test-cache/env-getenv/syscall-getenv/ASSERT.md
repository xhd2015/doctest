...
- Second captured run (different env B) still reports Cached > 0 because
  process env is **not** part of the leaf-cache key (same as os.Getenv / LookupEnv).
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
    ...
    if !stdoutHasPositiveCached(secondStdout) {
        t.Fatalf("second run lost cache after syscall.Getenv env change; env must not be in leaf-cache key:\n%s", secondStdout)
    }
}
```
