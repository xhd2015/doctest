...
- Second run is `0 Cached` after L3-only intermediate SETUP.md change
  (L0–L2 and leaf SETUP untouched).
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
    ...
    if !strings.Contains(state.SecondResp.Stdout, ", 0 Cached") {
        t.Fatalf("second run was cached after L3 SETUP.md edit; expected ', 0 Cached':\n%s", state.SecondResp.Stdout)
    }
}
```
