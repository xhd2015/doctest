...
**Layer L2 in-process** — `runner.ParseTestOptions` rejects `--color` + `--no-color`.
No product binary, no `label: e2e`.

## How to Run

```sh
doctest vet ./tests/test/color-flags/
doctest test ./tests/test/color-flags/
```
