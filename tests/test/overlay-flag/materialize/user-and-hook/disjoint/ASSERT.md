...
- `Replace[project-source] == "from-user"`.
- `Replace[active-vendor] == "from-hook"`.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if got := resp.OverlayReplace[resp.ProjectSource]; got != "from-user" {
		t.Fatalf("user key missing/wrong: got %q map=%v", got, resp.OverlayReplace)
	}
	if got := resp.OverlayReplace[resp.ActiveVendorSource]; got != "from-hook" {
		t.Fatalf("hook key missing/wrong: got %q map=%v", got, resp.OverlayReplace)
	}
}
```
