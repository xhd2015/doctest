...
**Feature**: `./tests/...` must not widen `--changed` runs because of sibling stray files

```
# only leaf_a ASSERT.md modified; leaf_b has unrelated untracked file
changed leaf_a/ASSERT.md + untracked leaf_b/stray.go -> doctest test --changed ./tests/... -> 1 Run
```
...
4. Run `doctest test --changed ./tests/...` from the repo root.
