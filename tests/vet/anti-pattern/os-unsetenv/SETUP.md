...
**Feature**: a doctest tree with a SETUP.md that calls `os.Unsetenv` (Parallel-unsafe process env mutation)
...
## Preconditions
- A doctest tree with a SETUP.md Go block that calls `os.Unsetenv`.

## Steps
1. Create a minimal doctest tree with a SETUP.md containing `os.Unsetenv(...)`.
