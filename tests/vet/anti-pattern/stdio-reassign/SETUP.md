...
**Feature**: a doctest tree with a SETUP.md that reassigns `os.Stdout` (Parallel-unsafe stdio mutation)
...
## Preconditions
- A doctest tree with a SETUP.md Go block that reassigns `os.Stdout` (same family: `os.Stderr`, `os.Stdin`).

## Steps
1. Create a minimal doctest tree with a SETUP.md containing `os.Stdout = ...`.
