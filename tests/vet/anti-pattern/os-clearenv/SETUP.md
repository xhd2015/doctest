...
**Feature**: a doctest tree with a SETUP.md that calls `os.Clearenv` (Parallel-unsafe process env mutation)
...
## Preconditions
- A doctest tree with a SETUP.md Go block that calls `os.Clearenv`.

## Steps
1. Create a minimal doctest tree with a SETUP.md containing `os.Clearenv()`.
