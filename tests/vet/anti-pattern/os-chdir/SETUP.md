...
**Feature**: a doctest tree with a SETUP.md that calls `os.Chdir` (Parallel-unsafe process cwd mutation)
...
## Preconditions
- A doctest tree with a SETUP.md Go block that calls `os.Chdir`.

## Steps
1. Create a minimal doctest tree with a SETUP.md containing `os.Chdir(...)`.
