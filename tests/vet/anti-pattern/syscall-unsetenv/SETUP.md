...
**Feature**: a doctest tree with a SETUP.md that calls `syscall.Unsetenv` (Parallel-unsafe process env mutation)
...
## Preconditions
- A doctest tree with a SETUP.md Go block that calls `syscall.Unsetenv`.

## Steps
1. Create a minimal doctest tree with a SETUP.md containing `syscall.Unsetenv(...)`.
