...
**Feature**: a doctest tree with a SETUP.md that calls `t.Chdir` (Parallel-unsafe testing helper cwd mutation)
...
## Preconditions
- A doctest tree with a SETUP.md Go block that calls `t.Chdir` (receiver ident `t`).

## Steps
1. Create a minimal doctest tree with a SETUP.md containing `t.Chdir(...)`.
