...
**Feature**: a doctest tree with a SETUP.md that calls `syscall.Setenv` (Parallel-unsafe process env mutation)
...
## Preconditions
- A doctest tree with a SETUP.md Go block that calls `syscall.Setenv` (same family: `syscall.Unsetenv`).

## Steps
1. Create a minimal doctest tree with a SETUP.md containing `syscall.Setenv(...)`.
