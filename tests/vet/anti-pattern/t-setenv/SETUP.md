...
**Feature**: a doctest tree with a SETUP.md that calls `t.Setenv` (Parallel-unsafe testing helper env mutation)
...
## Preconditions
- A doctest tree with a SETUP.md Go block that calls `t.Setenv` (receiver ident `t`).

## Steps
1. Create a minimal doctest tree with a SETUP.md containing `t.Setenv(...)`.
