...
- Exit non-zero: blank-assign + `return nil` is the same vacuous class as pure `return nil`.
- stderr tells authors to **remove** the Go **code block**.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if resp.ExitCode == 0 {
		t.Fatalf("expected nonzero exit for vacuous blank-assign Setup\nstdout:\n%s\nstderr:\n%s",
