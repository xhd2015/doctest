...
func TestLeftoverExposePreps(t *testing.T) {
	shared := runnerbuild.TreePrep{GenRoot: "/tmp/shared"}
	other := runnerbuild.TreePrep{GenRoot: "/tmp/other"}
	empty := runnerbuild.TreePrep{}
	got := leftoverExposePreps([]runnerbuild.TreePrep{shared, other, empty}, []runnerbuild.TreePrep{shared})
	...
	if leftover := leftoverExposePreps([]runnerbuild.TreePrep{shared, empty}, []runnerbuild.TreePrep{shared}); len(leftover) != 0 {
