...
func TestRunHelpOutput(t *testing.T) {
	tests := []struct {
		...
		{name: "vet help", args: []string{"vet", "--help"}, wants: []string{"Usage: doctest vet [-v|--verbose] [--changed] <dir...>", "-v", "--verbose", "--changed", "./...", "Validate"}},
		{name: "vet short help", args: []string{"vet", "-h"}, wants: []string{"Usage: doctest vet [-v|--verbose] [--changed] <dir...>", "./..."}},
		{name: "build help", args: []string{"build", "--help"}, wants: []string{"Usage: doctest build", "-v", "--verbose", "--rm", "--gen-dir", "-count", "--changed"}},
...
func TestRunErrorCases(t *testing.T) {
	tests := []struct {
		...
		{name: "skill missing action", args: []string{"skill", "doc-spec"}, wantErr: "skill requires doc-spec, code-spec, tdd, tdd-cli-agent, tdd-lite, reproduce, review, output-assert, implementer, or designer plus show or install"},
