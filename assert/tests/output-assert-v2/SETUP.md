...
**Feature**: Output Assert DSL v2 parse and match harness

```
# author supplies v2 YAML header + strict template body
Author -> Facade.Parse: template text
Facade -> v2 Parser (version: 2) or legacy_v1
...
- Facade dispatches v2 templates (YAML `version: 2`) to v2 implementation; others to `legacy_v1`.
- v2 has no `MatchContains` mode — strict full match only.
...
- `ParsedSummary` uses stable v2 DSL kinds (e.g. `Placeholder{PORT,number}`, `RegexLine`, `OmitLine{3}`, `Color gray`).
- v1 fallback leaves assert `ContainsBlock`, `LiteralLine`, or other legacy_v1 shapes.
- Color helpers produce ANSI bytes for `<ansi-color>` span matching (same SGR tokens as v1).
